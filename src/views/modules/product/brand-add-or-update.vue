...
export default {
  ...
  data() {
    ...
    var validateFirstLetter = (rule, value, callback) => {
      ...
      } else if (!/^[a-zA-Z]/.test(value) || value.length > 1) {
    ...
    return {
      ...
      dataRule: {
        ...
        firstLetter: [{required: true, validator: validateFirstLetter, trigger: "blur" }],
        sort: [{required: true, validator: validateSort, trigger: "blur" }],
      },
      fileList: [],
      action: "http://localhost:88/api/thirdpart/oss/upload",
    };
  },
  methods: {
    init(id) {
